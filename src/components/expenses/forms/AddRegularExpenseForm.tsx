...
import { RegularExpenseForm, RegularExpenseFormData } from "./RegularExpenseForm";

interface AddRegularExpenseFormProps {
    householdId: string;
    onSuccess: () => void;
    onCancel: () => void;
}

export const AddRegularExpenseForm = ({ householdId, onSuccess, onCancel }: AddRegularExpenseFormProps) => {
    const { user } = useAuth();
    const { toast } = useToast();
    const [saving, setSaving] = useState(false);

    const handleSubmit = async (data: RegularExpenseFormData) => {
        setSaving(true);
